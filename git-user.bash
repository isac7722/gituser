# ============================================================
# Git User Switcher — bash 4+ compatible version
# ============================================================
...
if ((BASH_VERSINFO[0] < 4)); then
  echo "gituser: bash 4.0 or later is required. (current: $BASH_VERSION)" >&2
  return 1
fi
...
function _gituser_load() {
  ...
  while IFS= read -r line; do
    ...
    # Register each alias in the map (bash IFS split)
    local alias_entry
    IFS=',' read -ra alias_list <<< "$raw_aliases"
    for alias_entry in "${alias_list[@]}"; do
      alias_entry="${alias_entry// /}"  # strip whitespace
...
function _gituser_help() {
  ...
  printf "  %-18s %s\n" "rule <sub>"         "Manage directory auto-switch rules"
...
function _gituser_rule_remove() {
  ...
  # Remove marker line + next 2 lines (includeIf block), including blank line
  local tmp_file
  tmp_file="$(mktemp)"
  awk -v marker="$marker" '
    /^$/ { blank=$0; next }
...
function _gituser_clone() {
  ...
  # If last extra_arg is not a flag, it may be the target directory
  local repo_dir
